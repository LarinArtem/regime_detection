...
class MarkovRegime:
    ...
    def _gaussian_likelihood(self, vol, regime):
        # If we are in the provided regime, how likely is it to see this vol
    ...
    def get_regime(self, bars):
        ...
        # Step 2 Compute emission likelihoods (Probability of observing the spot vol | regime)
...
class LiveMarketDashboard:
    ...
    def recalibrate_model(self):
        ...
        self.ib_app.historical_data.clear()
        self.ib_app.hist_done.clear()
        # request data for markov chain model calibration
    ...
    def draw_ohlc_chart(self):
        ...
        for i, bar in enumerate(bars):
            bg = Rectangle(
                ...
                1,
                y_max - y_min,
                facecolor = self.regime_model.bg_colors[bar.regime], # update with current regime
                edgecolor='none',
                alpha=0.4,
                zorder=0
            )
